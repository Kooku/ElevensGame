...
public class ElevensBoard extends Board {
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		if (selectedCards.size() == 2 && findPairSum11(selectedCards).size() > 1) {
			return true;
		} else if (selectedCards.size() == 3 && (findJQK(selectedCards).size() > 1)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean anotherPlayIsPossible() {
		...
		if ((findPairSum11(cIndexes).size() > 1) || (findJQK(cIndexes).size() > 1)) {
			return true;
		}
		return false;
	}
	...
	private ArrayList<Integer> findJQK(List<Integer> selectedCards) {
		...
		for (Integer kObj : selectedCards) {
			...
			if (cardAt(k).rank().equals("jack")) {
				indexes.add(selectedCards.get(c));
				c++;
				foundJack = true;
	...
	private boolean playPairSum11IfPossible() {
		if (!(findPairSum11(cardIndexes()).isEmpty())) {
	...
	private boolean playJQKIfPossible() {
		if (!(findJQK(cardIndexes()).isEmpty())) {
